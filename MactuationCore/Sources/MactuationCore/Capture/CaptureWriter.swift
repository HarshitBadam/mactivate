...
/// Writes one capture session directory. Not thread-safe; feed it from a
/// single serial queue (sources already deliver per-path in timestamp order).
public final class CaptureWriter {
    ...
    /// Flushes streams and writes `labels.csv` and `session.json`.
